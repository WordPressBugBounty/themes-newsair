                ...
                <div class="newsair-notice-content">
                    ...
                    <?php
                        echo wp_kses_post( sprintf(
                            ...
                            '<a href="https://wordpress.org/plugins/ansar-import" target="_blank">' . esc_html__('Ansar Import', 'newsair') . '</a>',
                            esc_html($theme_info->Name)
                        ) );
                        ?>
                    </p>
...
function newsair_admin_info_install_plugin() {
    ...
    if ( ! current_user_can( 'install_plugins' ) ) {
        wp_send_json_error( array( 'message' => __( 'Sorry, you are not allowed to access this page.', 'newsair' ) ), 403 );
    }

    // Security Nonce verification
    check_ajax_referer( 'newsair_install_plugin_nonce', 'security' );
    ...
    if ( ! file_exists( WP_PLUGIN_DIR . '/' . $plugin_slug ) ) {
        ...
        if ( is_wp_error( $api ) ) {
            wp_send_json_error( array( 'message' => __( 'Failed to fetch plugin information.', 'newsair' ) ) );
        }
        ...
        if ( is_wp_error( $result ) ) {
            wp_send_json_error( array( 'message' => __( 'Plugin installation failed.', 'newsair' ) ) );
        }
    }
    if ( current_user_can( 'activate_plugins' ) ) {
        activate_plugin( 'ansar-import/ansar-import.php' );
    }
    wp_send_json_success( array( 'message' => __( 'Plugin installed and activated successfully.', 'newsair' ) ) );
}